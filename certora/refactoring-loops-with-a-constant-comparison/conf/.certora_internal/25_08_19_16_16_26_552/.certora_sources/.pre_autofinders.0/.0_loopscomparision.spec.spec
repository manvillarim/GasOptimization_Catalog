...
definition couplingInvariant() returns bool = a.total == ao.total && a.rewardSum == ao.rewardSum && a.balanceSum == ao.balanceSum && a.isActive == ao.isActive && a.baseAmount == ao.baseAmount && a.tokens.length == ao.tokens.length && (forall uint256 i. (i < a.tokens.length => a.tokens[i] == ao.tokens[i])) && a.balances.length == ao.balances.length && (forall uint256 i. (i < a.balances.length => a.balances[i] == ao.balances[i])) && a.rewards.length == ao.rewards.length && (forall uint256 i. (i < a.rewards.length => a.rewards[i] == ao.rewards[i]));
...
function viewOptimizationCorrectness(method f, method g) {
    env eA;
    env eAo;
    calldataarg args;
    require eA == eAo && couplingInvariant();
    uint256 resultA = a.f(eA, args);
    uint256 resultAo = ao.g(eAo, args);
...
filtered { f -> f.selector == sig:a.complexConstantComparison().selector, g -> g.selector == sig:ao.complexConstantComparison().selector } {
    viewOptimizationCorrectness(f, g);
}
