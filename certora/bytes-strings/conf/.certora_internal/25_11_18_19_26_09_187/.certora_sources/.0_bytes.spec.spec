...
hook Sstore a.names[INDEX uint256 index] string newValue STORAGE {
    ghostNamesHashA[index] = keccak256(newValue);
}

hook Sstore ao.names[INDEX uint256 index] bytes newValue STORAGE {
    ghostNamesHashAo[index] = keccak256(newValue);
}

hook Sstore a.userNames[KEY address user] string newValue STORAGE {
    ghostUserNamesHashA[user] = keccak256(newValue);
}

hook Sstore ao.userNames[KEY address user] bytes newValue STORAGE {
