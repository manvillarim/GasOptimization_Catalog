...
definition couplingInvariant() returns bool =
    a.total == ao.total && 
    a.accumulator == ao.accumulator && 
    a.counter == ao.counter && 
    a.sumOfSquares == ao.sumOfSquares &&
    a.productTotal == ao.productTotal &&
    a.weightedSum == ao.weightedSum &&
    a.globalMultiplier == ao.globalMultiplier &&
    a.globalDivisor == ao.globalDivisor && 
    a.bonusThreshold == ao.bonusThreshold &&
    a.values.length == ao.values.length &&
    (forall uint256 i. (i < a.values.length => a.values[i] == ao.values[i])) &&
    a.weights.length == ao.weights.length && 
    (forall uint256 i. (i < a.weights.length => a.weights[i] == ao.weights[i])) &&
    a.users.length == ao.users.length &&
    (forall uint256 i. (i < a.users.length => a.users[i] == ao.users[i] &&
    a.userBalances[a.users[i]] == ao.userBalances[ao.users[i]]));
