...
contract A {
    ...
    function distributeRewards(uint256 rewardPerUser) external {
        uint256 distributed = 0;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000001,distributed)}
        
        for (uint256 i = 0; i < userList.length; i++) {
            address user = userList[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,user)}
            ...
            distributed += rewardPerUser;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000005,distributed)}
        }
    ...
    function processMultipleUsers(address[] calldata targets, uint256 bonus) external {
        for (uint256 i = 0; i < targets.length; i++) {
            address user = targets[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000004,user)}
    ...
    function getUserData(address user) external view returns (uint256, uint256, bool) {
        User memory u = users[user];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010002,0)}
