...
invariant arrayIntegrity()
    a.allUsers.length == a.totalUsers &&
    ao.allUsers.length == ao.totalUsers &&
    // All users in the array should be registered
    (forall uint256 i. i < a.allUsers.length => a.isRegistered[a.allUsers[i]]) &&
    (forall uint256 i. i < ao.allUsers.length => ao.isRegistered[ao.allUsers[i]]) &&
    // All registered users should have valid indices
    (forall address user. a.isRegistered[user] => (
        a.userIndex[user] < a.allUsers.length &&
        a.allUsers[a.userIndex[user]] == user
    )) &&
    (forall address user. ao.isRegistered[user] => (
        ao.userIndex[user] < ao.allUsers.length &&
        ao.allUsers[ao.userIndex[user]] == user
    ));