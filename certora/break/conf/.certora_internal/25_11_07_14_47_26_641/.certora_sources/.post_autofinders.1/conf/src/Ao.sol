...
contract Ao {
    ...
    function findFirstIncompleteTask(address user) external view returns (uint256) {
        for (uint256 i = 0; i < taskIds.length; i++) {
            uint256 taskId = taskIds[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,taskId)}
            Task memory task = tasks[taskId];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010004,0)}
    ...
    function findHighPriorityTask(uint256 minPriority) external view returns (uint256) {
        for (uint256 i = 0; i < taskIds.length; i++) {
            uint256 taskId = taskIds[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000005,taskId)}
    ...
    function getTaskData(uint256 taskId) external view returns (uint256, address, uint256, bool) {
        Task memory task = tasks[taskId];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010002,0)}
