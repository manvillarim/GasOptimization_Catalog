...
hook Sstore ao.exists[KEY uint256 j] bool newExists (bool oldExists) {
    ...
    if (!oldExists && newExists) {
        ...
        havoc compactToSparse assuming
            ...
            (compactToSparse@new[assert_uint256(old_len)] == j);
    ...
    if (oldExists && !newExists) {
        ...
        // Simplificação: assumir que o elemento removido está sempre na última posição
        // para evitar problemas com havoc de variáveis não-ghost
        havoc compactToSparse assuming
            (forall uint256 k. to_mathint(k) < old_len - 1 => 
                (compactToSparse@old[k] == j => 
                    compactToSparse@new[k] == compactToSparse@old[assert_uint256(old_len - 1)])) &&
            (forall uint256 k. to_mathint(k) < old_len - 1 => 
                (compactToSparse@old[k] != j => 
                    compactToSparse@new[k] == compactToSparse@old[k]));
