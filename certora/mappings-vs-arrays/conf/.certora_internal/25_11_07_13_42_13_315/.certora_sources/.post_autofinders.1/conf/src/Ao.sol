...
contract Ao {
    ...
    function addValue(uint256 _value) external returns (uint256 index) {
        ...
        index = size;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,index)}
        values[size] = _value;
        size++;
    ...
    function batchAdd(uint256[] calldata _values) external {
        require(size + _values.length <= type(uint256).max, "Size overflow");
