 ...
 * Defines when both contracts are in equivalent states
 */
definition couplingInv() returns bool =
    ...
    // 2. Logical length must be equal
    (ghostALength == ghostAoLength) &&
    (a.getLength() == ao.getLength()) &&
    
    // 3. For each valid index, active values must be equal
    (forall uint256 i. 
        (to_mathint(i) < ghostALength) =>
            (ghostAValues[i] == ghostAoValues[i] && ghostAoValues[i] != REMOVED_MARKER())
    ) &&
    
    // 4. All removed positions in Ao must be marked with REMOVED_MARKER
    // (This ensures no active value equals REMOVED_MARKER)
    (forall uint256 i.
