...
hook Sstore ao.values[INDEX uint256 j] uint256 newVal (uint256 oldVal) {
    if (!ao.exists[j] && newVal != 0) {
        // Novo elemento adicionado no índice j
        uint256 currentArrayLength = a.getLength();
...
hook Sstore ao.exists[INDEX uint256 j] bool newExists (bool oldExists) {
...
definition isKthActiveElement(uint256 j, uint256 k) returns bool =
    ao.exists(j) && 
    // Verifica via ghost mapping
    compactToSparse[k] == j;

// Coupling invariant corrigido usando ghost mapping
definition couplingInv() returns bool =
    ...
    (forall uint256 k. k < a.getLength() => 
        isKthActiveElement(compactToSparse[k], k) &&
        a.values(k) == ao.values(compactToSparse[k]));

// Invariante adicional: propriedades do ghost mapping
/*invariant mappingProperties()
    // O mapping é injetivo (sem duplicatas)
    (forall uint256 k1 uint256 k2. 
        k1 < a.getLength() && k2 < a.getLength() && k1 != k2 =>
        compactToSparse[k1] != compactToSparse[k2]) &&
    // Todos os índices mapeados devem existir em Ao
    (forall uint256 k. k < a.getLength() => 
        ao.exists(compactToSparse[k])) &&
    // Elementos deletados em Ao têm valor 0
    (forall uint256 j. !ao.exists(j) => ao.values(j) == 0);*/
