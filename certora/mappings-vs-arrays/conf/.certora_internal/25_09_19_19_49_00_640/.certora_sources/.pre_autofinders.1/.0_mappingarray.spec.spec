...
hook Sstore ao.exists[KEY uint256 j] bool newExists (bool oldExists) {
    ...
    if (!oldExists && newExists) {
        ...
        // Converter mathint para uint256 para indexação
        require old_len >= 0 && old_len <= max_uint256;
        uint256 old_len_uint = require_uint256(old_len);
        
        // Reordenar: primeiro a atribuição simples, depois o forall
        havoc compactToSparse assuming
            (compactToSparse@new[old_len_uint] == j) &&
            (forall uint256 k. k < old_len_uint => compactToSparse@new[k] == compactToSparse@old[k]);
