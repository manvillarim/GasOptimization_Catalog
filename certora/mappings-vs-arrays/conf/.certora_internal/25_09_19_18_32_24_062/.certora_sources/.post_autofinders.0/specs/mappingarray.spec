...
hook Sstore ao.exists[KEY uint256 j] bool newExists (bool oldExists) {
    ...
    if (!oldExists && newExists) { // Addition
        ...
        havoc compactToSparse assuming
            // CORREÇÃO: Removido @old
            (forall uint256 k. k < old_len => compactToSparse@new[k] == compactToSparse[k]) &&
    ...
    if (oldExists && !newExists) { // Removal
        uint256 k_removed;
        // CORREÇÃO: Removido @old
        require compactToSparse[k_removed] == j;

        havoc compactToSparse assuming forall uint256 k.
            // CORREÇÃO: Removido @old
            (k < k_removed => compactToSparse@new[k] == compactToSparse[k]) &&
            // CORREÇÃO: Removido @old e adicionado to_uint256()
            (k >= k_removed && k < ao.elementCount - 1 => compactToSparse@new[k] == compactToSparse[to_uint256(k + 1)]);
    }
}
