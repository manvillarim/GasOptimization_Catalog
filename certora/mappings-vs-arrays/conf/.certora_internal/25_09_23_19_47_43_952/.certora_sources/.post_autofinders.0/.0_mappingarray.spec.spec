...
/**
 * Coupling invariant between contracts A and Ao
 * Handles the different removal strategies:
 * - A: shifts elements and reduces length
 * - Ao: marks as removed but keeps logical structure
 */
definition couplingInv() returns bool =
    ...
    // 2. Element count relationships
    (ghostALength == ghostAElementCount) && // A: length always equals elementCount (compact)
    (ghostAoElementCount <= ghostAoLength) && // Ao: elementCount <= length (has removed slots)
    
    // 3. Both getLength() return active element count
    (a.getLength() == ao.getLength()) &&
    (ghostALength == to_mathint(a.getLength())) &&
    (ghostAoElementCount == to_mathint(ao.getLength())) &&
    
    // 4. Active values are equivalent (key insight for removeValue)
    // For every valid index in A, there exists a corresponding active value in Ao
    (ghostALength == ghostAoElementCount) && // Same number of active elements
    
    // 5. No value in Ao should equal REMOVED_MARKER unless it's actually removed
    (forall uint256 i.
        (to_mathint(i) < ghostAoLength && ghostAoValues[i] != REMOVED_MARKER()) =>
            (ghostAoValues[i] != REMOVED_MARKER())
    );

/**
 * Generic correctness function for method equivalence
 */
