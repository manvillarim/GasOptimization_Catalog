...
hook Sstore ao.exists[KEY uint256 j] bool newExists (bool oldExists) {
    ...
    if (oldExists && !newExists) {
        ...
        // encontrar o índice k_removed tal que compactToSparse@old[k_removed] == j
        uint256 k_removed_idx;
        havoc k_removed_idx assuming (k_removed_idx < old_len && compactToSparse@old[k_removed_idx] == j);

        // copiar prefixo e deslocar à esquerda o resto
        havoc compactToSparse assuming
            (forall uint256 k. k < k_removed_idx => compactToSparse@new[k] == compactToSparse@old[k]) &&
            (forall uint256 k. (k >= k_removed_idx && k < require_uint256(old_len - 1)) => 
                compactToSparse@new[k] == compactToSparse@old[require_uint256(k + 1)]);
        
        ghostElementCount = require_uint256(old_len - 1);
    }
}
...
definition couplingInv() returns bool =
    ...
    (ghostElementCount == ao.elementCount) &&
    ...
    (forall uint256 j.
        ao.exists[j] => (exists uint256 k. k < ghostElementCount && compactToSparse[k] == j)
    ) &&
    ...
    (forall uint256 k1. forall uint256 k2.
        (k1 < ghostElementCount && k2 < ghostElementCount && k1 != k2) =>
