...
contract Ao {
    ...
    function addValue(uint256 _value) external returns (uint256 index) {
        index = activeIndices.length;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000008,index)}
        values[index] = _value;
        activeIndices.push(index);
        indexPosition[index] = index;
    ...
    function getValue(uint256 _index) external view returns (uint256) {
        ...
        uint256 realIndex = activeIndices[_index];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000001,realIndex)}
    ...
    function updateValue(uint256 _index, uint256 _newValue) external {
        ...
        uint256 oldValue = values[realIndex];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,oldValue)}
    ...
    function removeValue(uint256 _index) external {
        ...
        uint256 realIndex = activeIndices[_index];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000004,realIndex)}
        uint256 removedValue = values[realIndex];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000005,removedValue)}
        
        // Get the last element's real index
        uint256 lastPos = activeIndices.length - 1;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000006,lastPos)}
        uint256 lastRealIndex = activeIndices[lastPos];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000007,lastRealIndex)}
        
        if (_index != lastPos) {
            // Move last element to removed position (array-like behavior)
            activeIndices[_index] = lastRealIndex;
            indexPosition[lastRealIndex] = _index;
        }
        
        // Clean up the removed element
    ...
    function getRangeSum(uint256 _start, uint256 _end) external view returns (uint256 sum) {
        ...
        for (uint256 i = _start; i <= _end; i++) {
            sum += values[activeIndices[i]];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000a,sum)}
        }
    }
    ...
    function batchAdd(uint256[] calldata _values) external {
        for (uint256 i = 0; i < _values.length; i++) {
            uint256 newIndex = activeIndices.length;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000009,newIndex)}
            values[newIndex] = _values[i];
            activeIndices.push(newIndex);
            indexPosition[newIndex] = newIndex;
