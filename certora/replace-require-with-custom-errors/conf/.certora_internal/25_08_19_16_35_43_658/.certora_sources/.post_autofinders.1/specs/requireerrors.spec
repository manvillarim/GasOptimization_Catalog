...
definition couplingInvariant() returns bool = a.owner == ao.owner && a.totalSupply == ao.totalSupply && a.maxSupply == ao.maxSupply && a.minTransferAmount == ao.minTransferAmount && a.maxTransferAmount == ao.maxTransferAmount && a.paused == ao.paused && a.minStakeAmount == ao.minStakeAmount && a.stakingDuration == ao.stakingDuration && (forall address u. a.balances[u] == ao.balances[u]) && (forall address u. forall address v. a.allowances[u][v] == ao.allowances[u][v]) && (forall address u. a.stakedBalances[u] == ao.stakedBalances[u]) && (forall address u. a.stakingTimestamps[u] == ao.stakingTimestamps[u]) && (forall address u. a.admins[u] == ao.admins[u]) && (forall address u. a.blacklisted[u] == ao.blacklisted[u]);
...
rule gasOptimizedCorrectnessOfEmergencyWithdraw(method f, method g)
filtered { f -> f.selector == sig:a.emergencyWithdraw(address, uint256).selector, g -> g.selector == sig:ao.emergencyWithdraw(address, uint256).selector } {
    gasOptimizationCorrectness(f, g);
}
