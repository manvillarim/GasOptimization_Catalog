...
definition couplingInv() returns bool =
    ...
    // Check all users array has same length and content
    (forall uint256 i. (i < a.totalUsers => (
        i < ao.totalUsers && a.allUsers[i] == ao.allUsers[i]
    ))) &&
...
// Invariant: total users count should match actual array length
invariant totalUsersConsistency()
    a.totalUsers == ao.totalUsers &&
    // For all indices less than totalUsers, the user at that index should be registered
    (forall uint256 i. i < a.totalUsers => (
