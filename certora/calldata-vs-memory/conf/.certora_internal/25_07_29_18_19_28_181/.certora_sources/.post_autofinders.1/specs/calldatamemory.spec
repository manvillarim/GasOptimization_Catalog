...
// Rule for batchProcess equivalence - using generic function
...
// Rule for complexCalculation equivalence - using generic function
...
// Rules for pure functions that only need to verify they return the same values
...
// Rule for pure functions with array returns - simplified
rule gasOptimizedCorrectnessOfPureFunctions(method f, method g)
filtered {
    f -> (f.selector == sig:a.findValues(uint256[],uint256[]).selector ||
          f.selector == sig:a.mergeSorted(uint256[],uint256[]).selector),
    g -> (g.selector == sig:ao.findValues(uint256[],uint256[]).selector ||
          g.selector == sig:ao.mergeSorted(uint256[],uint256[]).selector)
} {
    env eA;
    env eAo;
    calldataarg args;
    ...
    require eA == eAo;
    require f.selector == g.selector; // Ensure we're comparing the same function
    
    // Pure functions don't change state, so we just need to ensure
    // the coupling invariant remains true (which it should for pure functions)
    assert couplingInv();
}
...
invariant constructorInit()
    a.minValue == max_uint256 => ao.minValue == max_uint256;