...
methods {
    ...
    function a.transferOwnership(address) external;
    function ao.transferOwnership(address) external;
    
    function a.renounceOwnership() external;
    function ao.renounceOwnership() external;
...
rule gasOptimizedCorrectnessOfOwner(method f, method g)
filtered {
    f -> f.selector == sig:a.owner().selector,
    g -> g.selector == sig:ao.owner().selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfTransferOwnership(method f, method g)
filtered {
    f -> f.selector == sig:a.transferOwnership(address).selector,
    g -> g.selector == sig:ao.transferOwnership(address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfRenounceOwnership(method f, method g)
filtered {
    f -> f.selector == sig:a.renounceOwnership().selector,
    g -> g.selector == sig:ao.renounceOwnership().selector
} {
    gasOptimizationCorrectness(f, g);
}
