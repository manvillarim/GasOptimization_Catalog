...
methods {
    ...
    function a.getRevision() external returns (uint256) envfree;
    function ao.getRevision() external returns (uint256) envfree;
}
...
filtered {
    f -> f.selector == sig:a.setTransferStrategy(address, ITransferStrategyBase).selector,
    g -> g.selector == sig:ao.setTransferStrategy(address, ITransferStrategyBase).selector
} {
    gasOptimizationCorrectness(f, g);
}
...
filtered {
    f -> f.selector == sig:a.setRewardOracle(address, AggregatorInterface).selector,
    g -> g.selector == sig:ao.setRewardOracle(address, AggregatorInterface).selector
} {
    gasOptimizationCorrectness(f, g);
}
