...
methods {
    ...
    // Summary para IERC20.balanceOf - corrigido para usar currentContract
    function _.balanceOf(address account) external => ghostTokenBalance[calledContract][account] expect uint256 ALL;
    
    // Summary para sendValue (Address library) - sempre sucede
    function _.sendValue(address, uint256) external => NONDET;
...
ghost mapping(address => mapping(address => uint256)) ghostTokenBalance {
    init_state axiom forall address token. forall address user. 
        ghostTokenBalance[token][user] <= max_uint256;
}
...
// Ghosts para ______gap (53 slots de armazenamento reservado)
...
// Ghosts para AccessControl _roles mapping
// mapping(bytes32 => RoleData) private _roles;
// struct RoleData { mapping(address => bool) members; bytes32 adminRole; }
ghost mapping(bytes32 => mapping(address => bool)) ghost_a_roles_members;
ghost mapping(bytes32 => bytes32) ghost_a_roles_adminRole;

ghost mapping(bytes32 => mapping(address => bool)) ghost_ao_roles_members;
ghost mapping(bytes32 => bytes32) ghost_ao_roles_adminRole;

// Ghost para ReentrancyGuard _status
ghost uint256 ghost_a_reentrancyStatus {
    init_state axiom ghost_a_reentrancyStatus == 1; // NOT_ENTERED
}

ghost uint256 ghost_ao_reentrancyStatus {
    init_state axiom ghost_ao_reentrancyStatus == 1; // NOT_ENTERED
}
...
hook Sstore a.______gap[INDEX uint256 index] uint256 newValue (uint256 oldValue) {
    ghost_a_gap[index] = newValue;
}

hook Sload uint256 val a.______gap[INDEX uint256 index] {
    require ghost_a_gap[index] == val;
}
...
// Hooks para AccessControl _roles
hook Sstore a._roles[KEY bytes32 role].members[KEY address account] bool newValue (bool oldValue) {
    ghost_a_roles_members[role][account] = newValue;
}

hook Sload bool val a._roles[KEY bytes32 role].members[KEY address account] {
    require ghost_a_roles_members[role][account] == val;
}

hook Sstore a._roles[KEY bytes32 role].adminRole bytes32 newValue (bytes32 oldValue) {
    ghost_a_roles_adminRole[role] = newValue;
}

hook Sload bytes32 val a._roles[KEY bytes32 role].adminRole {
    require ghost_a_roles_adminRole[role] == val;
}

// Hook para ReentrancyGuard _status
hook Sstore a._status uint256 newValue (uint256 oldValue) {
    ghost_a_reentrancyStatus = newValue;
}

hook Sload uint256 val a._status {
    require ghost_a_reentrancyStatus == val;
}
...
// Hooks para ______gap
hook Sstore ao.______gap[INDEX uint256 index] uint256 newValue (uint256 oldValue) {
    ghost_ao_gap[index] = newValue;
}

hook Sload uint256 val ao.______gap[INDEX uint256 index] {
    require ghost_ao_gap[index] == val;
}
...
// Hooks para AccessControl _roles
hook Sstore ao._roles[KEY bytes32 role].members[KEY address account] bool newValue (bool oldValue) {
    ghost_ao_roles_members[role][account] = newValue;
}

hook Sload bool val ao._roles[KEY bytes32 role].members[KEY address account] {
    require ghost_ao_roles_members[role][account] == val;
}

hook Sstore ao._roles[KEY bytes32 role].adminRole bytes32 newValue (bytes32 oldValue) {
    ghost_ao_roles_adminRole[role] = newValue;
}

hook Sload bytes32 val ao._roles[KEY bytes32 role].adminRole {
    require ghost_ao_roles_adminRole[role] == val;
}

// Hook para ReentrancyGuard _status
hook Sstore ao._status uint256 newValue (uint256 oldValue) {
    ghost_ao_reentrancyStatus = newValue;
}

hook Sload uint256 val ao._status {
    require ghost_ao_reentrancyStatus == val;
}
...
definition couplingInv() returns bool =
    ...
    // ______gap deve ser igual (53 slots)
    (forall uint256 i. i < 53 => ghost_a_gap[i] == ghost_ao_gap[i]) &&
    
    // _streams mapping deve ser igual
    ...
    // AccessControl _roles deve ser igual
    (forall bytes32 role. forall address account.
        ghost_a_roles_members[role][account] == ghost_ao_roles_members[role][account]
    ) &&
    (forall bytes32 role.
        ghost_a_roles_adminRole[role] == ghost_ao_roles_adminRole[role]
    ) &&
    
    // ReentrancyGuard _status deve ser igual
...
rule gasOptimizationCorrectness(method f, method g) {
    env eA;
    env eAo;
    calldataarg args;
    ...
    require forall address token. 
        transferSuccess[token] == true;
    require forall address token.
        transferFromSuccess[token] == true;
...
// ====================================
// REGRAS DE VERIFICAÇÃO - TODAS AS FUNÇÕES
// ====================================
...
rule gasOptimizedCorrectnessOfGetStream(method f, method g)
filtered {
    f -> f.selector == sig:a.getStream(uint256).selector,
    g -> g.selector == sig:ao.getStream(uint256).selector
} {
    gasOptimizationCorrectness(f, g);
}