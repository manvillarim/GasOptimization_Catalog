...
methods {
    // Sumário para IScaledBalanceToken.scaledTotalSupply() 
    // Retorna um valor determinístico baseado no asset
    function _.scaledTotalSupply() external returns (uint256) => ghostScaledTotalSupply[calledContract];
...
// Ghost para scaledTotalSupply
ghost mapping(address => uint256) ghostScaledTotalSupply {
    init_state axiom forall address asset. ghostScaledTotalSupply[asset] >= 0;
}
...
// ====================================
// REGRAS DE VERIFICAÇÃO ESPECÍFICAS
// ====================================

// Ghost para rastrear totalSupply
ghost mapping(address => uint256) ghostTotalSupply {
    init_state axiom forall address asset. ghostTotalSupply[asset] == 0;
}

// Hook para garantir consistência do totalSupply
hook Sload uint256 val _assets[KEY address asset] {
    // Assumimos que o totalSupply é consistente entre os dois contratos
    require ghostTotalSupply[asset] >= 0;
}
