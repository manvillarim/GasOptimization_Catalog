...
methods {
    ...
    function _.sendValue(uint256) external => ALWAYS(true);
...
ghost mapping(uint256 => mapping(uint256 => uint256)) ghost_gap {
    init_state axiom forall uint256 contractId. forall uint256 i. 
        i < 53 => ghost_gap[contractId][i] == 0;
}
...
hook Sstore a.______gap[KEY uint256 idx] uint256 newValue (uint256 oldValue) {
    ghost_a_gap[idx] = newValue;
}

hook Sload uint256 val a.______gap[KEY uint256 idx] {
    require ghost_a_gap[idx] == val;
}
...
hook Sstore ao.______gap[KEY uint256 idx] uint256 newValue (uint256 oldValue) {
    ghost_ao_gap[idx] = newValue;
}

hook Sload uint256 val ao.______gap[KEY uint256 idx] {
    require ghost_ao_gap[idx] == val;
}
