...
methods {
    ...
    function _.transfer(address, uint256) external => ghostTransferSuccess[calledContract];
    
    // Summary para IERC20.transferFrom
    function _.transferFrom(address, address, uint256) external => ghostTransferFromSuccess[calledContract];
    
    // Summary para SafeERC20.safeTransfer
    function _.safeTransfer(address, uint256) external => ALWAYS(true);
    
    // Summary para SafeERC20.forceApprove  
...
// Ghost para o array ______gap usando mapping indexado
...
// Hook genérico para array ______gap usando STORAGE
...
definition couplingInv() returns bool =
    // Comparação do _nextStreamId
    ghost_a_nextStreamId == ghost_ao_nextStreamId &&
    
    // Comparação do array ______gap  
    (forall uint256 i. i < 53 => ghost_a_gap[i] == ghost_ao_gap[i]) &&
    
    // Comparação de todos os campos das streams
