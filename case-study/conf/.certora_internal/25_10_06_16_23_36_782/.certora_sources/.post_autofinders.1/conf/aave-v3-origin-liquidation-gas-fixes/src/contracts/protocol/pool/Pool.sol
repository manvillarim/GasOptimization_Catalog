...
import {PoolLogic as PoolLogicOptimized} from '../libraries/logic/PoolLogic.sol';
import {ReserveLogic as ReserveLogicOptimized} from '../libraries/logic/ReserveLogic.sol';
import {EModeLogic as EModeLogicOptimized} from '../libraries/logic/EModeLogic.sol';
...
abstract contract Pool is VersionedInitializable, PoolStorage, IPool {
  using ReserveLogicOptimized for DataTypes.ReserveData;
  ...
  function mintToTreasury(address[] calldata assets) external virtual override {
    PoolLogicOptimized.executeMintToTreasury(_reserves, assets);
  }
  ...
  {
    return
      PoolLogicOptimized.executeGetUserAccountData(
        _reserves,
        _reservesList,
        _eModeCategories,
  ...
  function initReserve(
    ...
    if (
      PoolLogicOptimized.executeInitReserve(
        _reserves,
        _reservesList,
  ...
  function dropReserve(address asset) external virtual override onlyPoolConfigurator {
    PoolLogicOptimized.executeDropReserve(_reserves, _reservesList, asset);
  }
  ...
  function syncRatesState(address asset) external virtual override onlyPoolConfigurator {
    ...
    ReserveLogicOptimized.updateInterestRatesAndVirtualBalance(reserve, reserveCache, asset, 0, 0);
  }
  ...
  function setUserEMode(uint8 categoryId) external virtual override {
    EModeLogicOptimized.executeSetUserEMode(
      _reserves,
      _reservesList,
      _eModeCategories,
      _usersEModeCategory,
  ...
  function resetIsolationModeTotalDebt(
    ...
    PoolLogicOptimized.executeResetIsolationModeTotalDebt(_reserves, asset);
  }
  ...
  function setLiquidationGracePeriod(
    ...
    require(_reserves[asset].id != 0 || _reservesList[0] == asset, Errors.ASSET_NOT_LISTED);
    PoolLogicOptimized.executeSetLiquidationGracePeriod(_reserves, asset, until);
  }
  ...
  function rescueTokens(
    ...
    PoolLogicOptimized.executeRescueTokens(token, to, amount);
  }
  ...
  function getEModeLogic() external pure returns (address) {
    return address(EModeLogicOptimized);
  }
  ...
  function getPoolLogic() external pure returns (address) {
    return address(PoolLogicOptimized);
  }
